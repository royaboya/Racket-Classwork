...
;; if cross pipe is already propogated, (i.e check if it exists in goo flow path or something), then if it flows
;; into that same cross pipe pass in the "none" direction to make it full

;; pipe->image: Pipe Integer Integer Direction -> Image
...
(define (pipe->image pipe tile-side-length pipe-width filled? cross-dir)
  (local [ ;; gray frame of pipe
    ...
    (cond
      ...
      ;; cross vert
      [(and (pipe-top pipe) (pipe-bot pipe) (pipe-left pipe) (pipe-right pipe) filled? (or (string=? TOP cross-dir) (string=? BOTTOM cross-dir)))
       (overlay/align "middle" "center" (rectangle pipe-width tile-side-length "solid" "green")
                      (overlay/align "middle" "center" (rectangle tile-side-length pipe-width "solid" "black") PS))]

      ;; cross hor
      [(and (pipe-top pipe) (pipe-bot pipe) (pipe-left pipe) (pipe-right pipe) filled? (or (string=? LEFT cross-dir) (string=? RIGHT cross-dir)))
       (overlay/align "middle" "center" (overlay/align "middle" "center" (rectangle tile-side-length pipe-width "solid" "green")
...
;; - "right"
;; - "none"
;; Represents a direction on a grid or pipe, "none" for when direction is not needed
...
(define-struct placed-pipe [pipe row col fill dir]) 
;; A Placed-Pipe is a (make-placed-pipe Pipe Number Number Direction)
...
;; EXAMPLES
(define P1 (make-placed-pipe PIPE-TL 1 2 #f NONE))
(define P2 (make-placed-pipe PIPE-TB 3 1 #f NONE))
(define P3 (make-placed-pipe PIPE-CROSS 2 2 #f NONE))
...
;; More examples
(define GRID-1 (make-grid 6 (list (make-placed-pipe PIPE-TL 1 2 #f NONE))))
(define GRID-2 (make-grid 3 (list (make-placed-pipe PIPE-TL 1 1 #f NONE))))
(define GRID-3 (make-grid 9 (list (make-placed-pipe PIPE-TL 3 3 #f NONE))))
...
(define (place-pipe grid pipe row col fill dir)
  (make-grid (grid-dimension grid) (cons (make-placed-pipe pipe row col fill dir) (grid-pipes grid))))

;; check expects:
(check-expect (place-pipe STARTING-GRID PIPE-TL 1 1 #f NONE) (make-grid 7 (list (make-placed-pipe PIPE-TL 1 1 #f NONE))))

(check-expect (place-pipe GRID-1 PIPE-TL 1 2 #f NONE) (make-grid 6 (list (make-placed-pipe PIPE-TL 1 2 #f NONE) (make-placed-pipe PIPE-TL 1 2 #f NONE))))

(check-expect (place-pipe GRID-2 PIPE-TR 2 4 #f NONE) (make-grid 3 (list (make-placed-pipe PIPE-TR 2 4 #f NONE) (make-placed-pipe PIPE-TL 1 1 #f NONE))))
...
(define (placed-pipe-loc grid row col) ;; row is y, col is x
  (cond
    [(empty? (grid-pipes grid)) #false]
    [(and (= row (placed-pipe-row (first (grid-pipes grid))))
          (= col (placed-pipe-col (first (grid-pipes grid)))))
      (first (grid-pipes grid))]
    [else (placed-pipe-loc (make-grid (grid-dimension grid) (rest (grid-pipes grid))) row col)]))
...
(define (grid->image grid tile-side-length pipe-width incoming score)
  (local[
         (define ROW (row-tile tile-side-length (grid-dimension grid)))
         (define GRID (grid-from-row ROW (grid-dimension grid)))
         (define GRID/v1 (above/align "left" GRID (draw-incoming-pipes 4 incoming tile-side-length pipe-width)))
         (define GRID/v2 (above/align "right" GRID/v1 (text (string-append "(Task 6 on-tick is inconsistent, it might take a while) score: " (number->string score)) 20 "black")))
         ]
    ...
    (cond
      ...
      [else (overlay/align/offset "left" "top"
                                  (pipe->image
                                   ...
                                   tile-side-length
                                   pipe-width
                                   (placed-pipe-fill (first (grid-pipes grid))) (placed-pipe-dir (first (grid-pipes grid))))
                                  (- 0 (* tile-side-length (placed-pipe-row (first (grid-pipes grid)))))
                                  (- 0 (* tile-side-length (placed-pipe-col (first (grid-pipes grid)))))
                                  (grid->image (make-grid (grid-dimension grid) (rest (grid-pipes grid))) tile-side-length pipe-width incoming score))])))
...
(define (draw-incoming-pipes n pipe-list tl pw)
  (local[
    ...
    (cond
      ...
      [(and (not (empty? pipe-list)) (not (= 1 n))) (beside (pipe->image (first pipe-list) tl pw #f NONE)
                                                            (draw-incoming-pipes (- n 1) (rest pipe-list) tl pw))]
      [(and (empty? pipe-list) (= 1 n)) EMPTY]
      [(= 1 n) (pipe->image (first pipe-list) tl pw #f NONE)])))


(define-struct game-state [grid pipes-to-place tile-side-length pipe-width start goo-flow replaced goo-length ticks])
...
;; - goo-length is a tracker of the GooFlow path length
...
(define (gamestate-init n x y dir pipe-list tl pw)
  (local[
         ...
         (define g (make-grid n (list (make-placed-pipe (dir->start dir) x y #f NONE))))
         ]
    (make-game-state g pipe-list tl pw PIPE-TYPE (dir->gooflow dir) 0 0 60)))

;; game-state-init check-expects
#|
(check-expect (gamestate-init 5 0 0 LEFT '() 100 30) (make-game-state
                     (make-grid 5 (list (make-placed-pipe (make-pipe #false #false #true #false) 0 0 #false NONE)))
                     '()
                     100
                     30
...
(check-expect (gamestate-init
               ...
               '() 100 30) (make-game-state
                     (make-grid 2 (list (make-placed-pipe (make-pipe #false #true #false #false) 0 0 #false NONE)))
                     '()
                     100
                     30
...
(check-expect (gamestate-init
               ...
               (list PIPE-TR PIPE-LR) 100 30)(make-game-state
                                       (make-grid 3 (list (make-placed-pipe (make-pipe #false #true #false #false) 0 0 #false NONE)))
         ...
         (define (add-filled-pipe/v2 gs)
           (local[
             ...
             (make-game-state
              (place-pipe (game-state-grid gs)
                          ...
                          (posn-y (first (goo-flow-path gf)))
                          #t (if (= (length (goo-flow-path gf)) 1)
                                 (goo->dir gf (game-state-grid gs))
                                 (calculate-dir (first (goo-flow-path gf)) (second (goo-flow-path gf)) (game-state-grid gs))
                                 ))
              ...
              (game-state-replaced gs) ;; maybe add 1 to replaced here?
              (add1 (game-state-goo-length gs))
              (game-state-ticks gs)) ))
          ...
          (define (useless gs)
             (make-game-state
              ...
              (game-state-goo-length gs)
              (game-state-ticks gs)))
         ]
    
    ; (grid-goo-propagate (game-state-grid gs) (game-state-goo-flow gs))
    (cond
      ;(and (empty? (game-state-pipes-to-place gs)) (mouse=? me "button-down")) (add-filled-pipe/v2 (propogate-finish gs))
      [(empty? (goo-flow-path (game-state-goo-flow gs))) gs]
      [(= 0 (game-state-ticks gs)) (add-filled-pipe/v2 (useless gs))]
      
      ;[(and (empty? (game-state-pipes-to-place gs)) (mouse=? me "button-down")) (add-filled-pipe/v2 gs)]
      ...
      ; (if (boolean? (placed-pipe-loc (game-state-grid gs) (floor (/ x SCALING-FACTOR)) (floor (/ y SCALING-FACTOR))))
      [(and (mouse=? me "button-down")
            (placed-pipe? (placed-pipe-loc
                           (game-state-grid gs)
                           (floor (/ x SCALING-FACTOR))
                           (floor (/ y SCALING-FACTOR))))
            (placed-pipe-fill (placed-pipe-loc
                           (game-state-grid gs)
                           (floor (/ x SCALING-FACTOR))
                           (floor (/ y SCALING-FACTOR)))))
             gs] ;check for pipe existence, then get values 
      [(mouse=? me "button-down") ;; add function check here
       (make-game-state
        (place-pipe (game-state-grid gs)
                    (first (game-state-pipes-to-place gs))
                    (floor (/ x SCALING-FACTOR)) (floor (/ y SCALING-FACTOR)) #f NONE) 
        ...
        (if (boolean? (pipe-at
                    ...
                    (floor (/ x SCALING-FACTOR))
                    (floor (/ y SCALING-FACTOR))))
            (game-state-replaced gs) (add1 (game-state-replaced gs)))
        ;; maybe add one to replaced here after checks, i.e function that checks if pipe is already placed -> bool
        (game-state-goo-length gs)
        (game-state-ticks gs))]
...
(define (draw-game gs)
  ...
  #;(cond
    ...
    [(<= (length (goo-flow-path (game-state-goo-flow gs))) 2) (grid->image (game-state-grid gs)
                       (game-state-tile-side-length gs)
                       (game-state-pipe-width gs)
                       (game-state-pipes-to-place gs)
                       )]
    ...
    [else (grid->image
           ...
           (game-state-pipe-width gs)
           (game-state-pipes-to-place gs)
           (calculate-dir (first (goo-flow-path (game-state-goo-flow gs)))
                          (second (goo-flow-path(game-state-goo-flow gs)))))]

    
    )
  )

;; calculate-dir: Posn Posn Grid -> Direction
;; Calculates the moving direction given location of two pipes
(define (calculate-dir p1 p2 grid)
  (local[
         (define x1 (posn-x p1))
         (define x2 (posn-x p2))
         (define y1 (posn-y p1))
         ...
         (define PT2 (placed-pipe-loc grid (posn-x p1) (posn-y p1)))
         ;(define PT2 (placed-pipe-at grid 1 2))
         ]
  (cond
    [(and (placed-pipe? PT2)
          (pipe-top (placed-pipe-pipe PT2))
          (pipe-bot (placed-pipe-pipe PT2))
          (pipe-left (placed-pipe-pipe PT2))
          (pipe-right (placed-pipe-pipe PT2))
          (ormap (λ (s) (string=? s (placed-pipe-dir PT2))) (list "top" "left" "right" "bottom"))) "none"] ;; remove equal in future
    [ (= -1 x-diff) "right"]
    [(= 1 x-diff) "left"]
...
;; goo->dir: GooFlow Grid -> Direction
;; Calculates the direction from a GooFlow
(define (goo->dir gf grid)
  (local[
         (define PT2 (placed-pipe-loc grid (posn-y (first (goo-flow-path gf))) (posn-x (first (goo-flow-path gf)))))
  ...
  (cond
    [(and
      (pipe? PT2)
          ...
          (pipe-left (placed-pipe-pipe PT2))
          (pipe-right (placed-pipe-pipe PT2))
          (ormap (λ (s) (string=? s (placed-pipe-dir PT2))) (list "top" "bottom" "left" "right"))
          ) "none"]
    
    [(goo-flow-top gf) "top"]
    [(goo-flow-bot gf) "bottom"]
    [(goo-flow-left gf) "left"]
    [(goo-flow-right gf) "right"])))


;; get-score: GameState -> Integer
;; Calculates the score of a Pipe Fantasy Game
(define (get-score gs)
  (* 50 (- (game-state-goo-length gs) (game-state-replaced gs))))

(check-expect (get-score STATE-ONE) 0)
(check-expect (get-score STATE-TWO) 400)
(check-expect (get-score STATE-THREE) -350)
...
;; tick-handler: Game-State -> GameState
;; Handles the gamestate ticks every tick
(define (tick-handler gs)
  (cond
    [(= (game-state-ticks gs) 0)
       (make-game-state (game-state-grid gs)
                   ...
                   (game-state-goo-flow gs)
                   ;(grid-goo-propagate (game-state-goo-flow gs) (game-state-grid gs))
                   ;(game-state-goo-flow (propogate-finish gs))
                   (game-state-replaced gs)
                   (game-state-goo-length gs)
                   20)] ;; using 20 as tick because 28 is like even slower for some reason

    
    [else (make-game-state (game-state-grid gs)
                   (game-state-pipes-to-place gs)
                   (game-state-tile-side-length gs)
                   (game-state-pipe-width gs)
                   (game-state-start gs)
                   ...
                   (game-state-replaced gs)
                   (game-state-goo-length gs)
                   (sub1 (game-state-ticks gs)))]))

;; Some Game Demos 
(define STATE-FOUR (gamestate-init 6 1 1 RIGHT (list PIPE-BL PIPE-CROSS PIPE-TB PIPE-TL PIPE-TR PIPE-CROSS PIPE-BR PIPE-LR PIPE-TL) 100 30))
;(pipe-fantasy STATE-FOUR)

(define STATE-FIVE (gamestate-init 5 0 1 RIGHT '() 100 30))
;(pipe-fantasy STATE-FIVE)

(define G (make-grid
  6
  (list
   (make-placed-pipe (make-pipe #true #false #true #false) 4 2 #false "none")
   (make-placed-pipe (make-pipe #false #false #true #true) 3 2 #false "none")
   (make-placed-pipe (make-pipe #false #true #false #true) 1 2 #false "none")
   (make-placed-pipe (make-pipe #true #true #true #true) 1 3 #false "none")
   (make-placed-pipe (make-pipe #true #false #false #true) 1 4 #false "none")
   (make-placed-pipe (make-pipe #true #false #true #false) 2 4 #false "none")
   (make-placed-pipe (make-pipe #true #true #false #false) 2 3 #false "none")
   (make-placed-pipe (make-pipe #true #true #true #true) 2 2 #false "none")
   (make-placed-pipe (make-pipe #false #true #true #false) 2 1 #false "none")
   (make-placed-pipe (make-pipe #false #false #false #true) 1 1 #false "none"))))

(define DEMO (make-game-state G
                              (list PIPE-BR PIPE-TB PIPE-LR PIPE-CROSS PIPE-TL)
                              100
                              30
                              PIPE-START-RIGHT
                              (make-goo-flow #f #f #f #t (list
                                                          (make-posn 1 1)
                                                          (make-posn 2 1)
                                                          (make-posn 2 2)
                                                          (make-posn 2 3)
